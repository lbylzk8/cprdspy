...
def ConcentricCircles(center, n, d, radius, color='b'):
    fig = go.Figure()
    fig.add_trace(go.Scatter(x=[center[0]], y=[center[1]], mode='markers', marker=dict(color=color)))
    for i in range(n):
...
def wave_circle_ari_o(A, F, P, color, theta=0, R=1):
    ...
    for i in range(P+1):
        sub_fig = ConcentricCircles_o((np.cos(i*2*np.pi/P+np.pi/2+theta),
...
def wave_circle_ari_i(A, F, P, color, theta=0, R=1):
    ...
    for i in range(P+1):
        sub_fig = ConcentricCircles_i((np.cos(i*2*np.pi/P+np.pi/2+theta),
                            np.sin(i*2*np.pi/P+np.pi/2+theta)), F, A, R, color)
        fig.add_traces(sub_fig.data)
    fig.update_layout(showlegend=False)
    fig.update_yaxes(scaleanchor="x", scaleratio=1)
...
def wave_circle_ari(A, F, P, color, theta=0, R=1):
    ...
    for i in range(P+1):
        sub_fig = ConcentricCircles((np.cos(i*2*np.pi/P+np.pi/2+theta),
                           np.sin(i*2*np.pi/P+np.pi/2+theta)), F, A, R, color)
        fig.add_traces(sub_fig.data)
    fig.update_layout(showlegend=False)
    fig.update_yaxes(scaleanchor="x", scaleratio=1)
...
def n_flower_petal(center, R, r, n, theta=0, color='b'):
    ...
    center1 = (np.cos(theta+alpha/2)*R + center[0], np.sin(theta+alpha/2)*R+center[1])
    center2 = (np.cos(theta+alpha/2-2*np.pi/n)*R + center[0], np.sin(theta+alpha/2-2*np.pi/n)*R+center[1])
    fig = go.Figure()
    if abs(r - a) < 1e-12:
        arc1 = arc_degree_p(center1, r, np.pi+alpha/2+theta, np.pi+alpha/2+theta+theta_arc)
        arc2 = arc_degree_p(center2, r, np.pi/2+theta, np.pi/2+theta+theta_arc)
    elif r > a:
        arc1 = arc_degree_p(center1, r, np.pi+alpha/2+theta, np.pi+alpha/2+theta+theta_arc)
        arc2 = arc_degree_p(center2, r, np.pi/2-beta+theta, np.pi/2-beta+theta+theta_arc)
    else:
        print("r=", r, ",a=", a)
        print('r<a,不能形成花瓣。')
        return fig
    fig.add_trace(go.Scatter(x=arc1[0], y=arc1[1], mode='lines', line=dict(color=color)))
    fig.add_trace(go.Scatter(x=arc2[0], y=arc2[1], mode='lines', line=dict(color=color)))
    fig.update_yaxes(scaleanchor="x", scaleratio=1)
    return fig
...
def n_flower_arc(center, R, r, n, theta=0, color='b'):
    ...
    fig = go.Figure()
    if abs(r - a) < 1e-12:
        arc1 = arc_degree_p(center1, r, np.pi/2+alpha+theta, np.pi/2+alpha+theta+theta_petal)
        arc2 = arc_degree_p(center2, r, np.pi/2+theta, np.pi/2+theta+theta_petal)
    elif r > a:
        arc1 = arc_degree_p(center1, r, np.pi/2+alpha-beta+theta, np.pi/2+alpha-beta+theta+theta_petal)
        arc2 = arc_degree_p(center2, r, np.pi/2-beta+theta, np.pi/2-beta+theta+theta_petal)
    else:
        print("r=", r, ",a=", a)
        print('r<a,不能形成花瓣。')
        return fig
...
def n_flowers_flower_arc_with_field(center, R, r, n, theta=0, color='b', colorfield='#ff0'):
    ...
    if abs(r - a) < 1e-12:
        arc1 = arc_degree_p(center1, r, np.pi/2+alpha+theta, np.pi/2+alpha+theta+theta_petal)
        arc2 = arc_degree_p(center2, r, np.pi/2+theta, np.pi/2+theta+theta_petal)
        arc1_inv = arc_degree_p_inverse(center1, r, np.pi/2+alpha+theta, np.pi/2+alpha+theta+theta_petal)
        arc2_inv = arc_degree_p_inverse(center2, r, np.pi/2+theta, np.pi/2+theta+theta_petal)
    elif r > a:
        arc1 = arc_degree_p(center1, r, np.pi/2+alpha-beta+theta, np.pi/2+alpha-beta+theta+theta_petal)
        arc2 = arc_degree_p(center2, r, np.pi/2-beta+theta, np.pi/2-beta+theta+theta_petal)
        arc1_inv = arc_degree_p_inverse(center1, r, np.pi/2+alpha-beta+theta, np.pi/2+alpha-beta+theta+theta_petal)
        arc2_inv = arc_degree_p_inverse(center2, r, np.pi/2-beta+theta, np.pi/2-beta+theta+theta_petal)
    else:
        print("r=", r, ",a=", a)
        print('r<a,不能形成花瓣。')
        return fig
    fig.add_trace(go.Scatter(x=arc1[0], y=arc1[1], mode='lines', line=dict(color=color)))
    fig.add_trace(go.Scatter(x=arc2[0], y=arc2[1], mode='lines', line=dict(color=color)))
    fig.add_trace(go.Scatter(x=arc1_inv[0], y=arc1_inv[1], mode='lines', line=dict(color=colorfield)))
    fig.add_trace(go.Scatter(x=arc2_inv[0], y=arc2_inv[1], mode='lines', line=dict(color=colorfield)))
    fig.update_yaxes(scaleanchor="x", scaleratio=1)
    return fig
...
def one_flower_flower_arc_with_field(center, R, r, n, theta=0, color='b', colorfield='#ff0'):
    return n_flowers_flower_arc_with_field(center, R, r, n, theta+np.pi/2, color, colorfield)
...
def flowers_flower_by_flower_arc_with_field(center, R, r, N, n, theta, color='b', colorfield='#ff0'):
    fig = go.Figure()
    for i in range(0, N):
        sub_fig = one_flower_flower_arc_with_field(center, R, r, n, 2*i*np.pi/N+theta, color, colorfield)
        fig.add_traces(sub_fig.data)
    fig.update_yaxes(scaleanchor="x", scaleratio=1)
    return fig
...
def flowers_flower_by_petal_multi(center, R, r, n, ratio, M, N, theta, color='b'):
    ...
    for j in range(1, M+1):
        for i in range(0, N):
            sub_fig = one_flower_petal(center, R*(ratio**(j-1)), r*(ratio**(j-1)), n, 2*i*np.pi/N+(j-1)*np.pi/N+theta, color)
            fig.add_traces(sub_fig.data)
    fig.update_yaxes(scaleanchor="x", scaleratio=1)
    return fig
...
def flower_by_petal_fill(center, r, M, N, n, theta, colorf='r', color='b'):
    ...
    for j in range(1, M+1):
        for i in range(0, N):
            sub_fig = one_petal_fill(center, (np.sqrt(2*np.cos(np.pi/n))**(2*j-1)*r), n, 2*i*np.pi/N+(j-1)*np.pi/N+theta, colorf, color)
            fig.add_traces(sub_fig.data)
    fig.update_yaxes(scaleanchor="x", scaleratio=1)
    return fig
...
def one_flower_petal_fill(center, R, r, n, theta=0, colorf='r', color='b', alpha=0.5):
    return n_flowers_petal_fill(center, R, r, n, theta+np.pi/2, colorf, color, alpha)
...
def flowers_flower_by_petal_fill(center, R, r, N, n, theta, colorf='r', color='b', alpha=0.5):
    fig = go.Figure()
    for i in range(0, N):
        sub_fig = one_flower_petal_fill(center, R, r, n, 2*i * np.pi/N+theta, colorf, color, alpha)
        fig.add_traces(sub_fig.data)
    fig.update_yaxes(scaleanchor="x", scaleratio=1)
    return fig
...
def n_lily_petal_fill(center, r, n, theta, colorf='b', color='r', alpha=0.1):
    alpha = np.pi/n
    beta = np.pi-np.pi/n
    center1 = (np.cos(theta+alpha)*r + center[0], np.sin(theta+alpha)*r + center[1])
    center2 = (np.cos(theta+alpha-2*np.pi/n)*r + center[0], np.sin(theta+alpha-2*np.pi/n)*r + center[1])
    arc = arc_degree_p(center1, r, alpha+np.pi+theta, alpha+np.pi+theta+np.pi*(n-2)/n)
    arc_inverse = arc_degree_p_inverse(center2, r, beta+theta, beta+theta+np.pi*(n+2)/n)
    x = arc[0]
    y = arc[1]
    x1 = arc_inverse[0]
    y1 = arc_inverse[1]
    fig = go.Figure()
    fig.add_trace(go.Scatter(x=x, y=y, fill='toself', fillcolor=colorf, line=dict(color=color, width=alpha)))
    fig.add_trace(go.Scatter(x=x1, y=y1, fill='toself', fillcolor=colorf, line=dict(color=color, width=alpha)))
    fig.update_yaxes(scaleanchor="x", scaleratio=1)
    return fig
...
def n_flowers_petal_fill(center, R, r, n, theta=0, colorf='b', color='r', alpha=0.1):
    ...
    center2 = (np.cos(theta+alpha/2-2*np.pi/n)*R + center[0], np.sin(theta+alpha/2-2*np.pi/n)*R + center[1])
    if abs(r - a) < 1e-12:
        arc = arc_degree_p(center1, r, np.pi+alpha/2+theta, np.pi+alpha/2+theta+theta_arc)
        arc_inverse = arc_degree_p(center2, r, np.pi/2+theta, np.pi/2+theta+theta_arc)
    elif r > a:
        arc = arc_degree_p(center1, r, np.pi+alpha/2+theta, np.pi+alpha/2+theta+theta_arc)
        arc_inverse = arc_degree_p(center2, r, np.pi/2-beta+theta, np.pi/2-beta+theta+theta_arc)
    elif r < a:
        print("r=", r, ",a=", a)
        print('r<a,不能形成花瓣。')
        return go.Figure()
    ...
    fig = go.Figure()
    fig.add_trace(go.Scatter(x=merged_x, y=merged_y, fill='toself', fillcolor=colorf, line=dict(color=color, width=alpha)))
    fig.update_yaxes(scaleanchor="x", scaleratio=1)
    return fig
...
def rodincoil(R, r, n, color='b', theta=0):
    fig = go.Figure()
    for i in range(0, n):
        sub_fig = circle((R*np.cos(i*2*np.pi/n+theta), R*np.sin(i*2*np.pi/n+theta)), r, color)
        fig.add_traces(sub_fig.data)
    fig.update_yaxes(scaleanchor="x", scaleratio=1)
    return fig
...
def rodincoil_colorful(R, r, n, colors, theta=0):
    fig = go.Figure()
    for i in range(0, n):
        sub_fig = circle((R*np.cos(i*2*np.pi/n+theta), R*np.sin(i*2*np.pi/n+theta)), r, colors[i])
        fig.add_traces(sub_fig.data)
    fig.update_yaxes(scaleanchor="x", scaleratio=1)
    return fig
...
def draw_points(points, colorp='b', size=10):
    fig = go.Figure()
    for point in points:
        fig.add_trace(go.Scatter(x=[point[0]], y=[point[1]], mode='markers', marker=dict(color=colorp, size=size)))
    fig.update_yaxes(scaleanchor="x", scaleratio=1)
    return fig
...
def swastika(N, R=1, theta=0):
    points = [(R*np.sqrt(2)**i*np.cos(i*np.pi/4+theta), R*np.sqrt(2)**i*np.sin(i*np.pi/4+theta)) for i in range(N)]
    return points
...
def draw_swastika_plotly(N, R=1, theta=0, color='blue'):
    ...
    for i in range(len(points)):
        fig.add_trace(go.Scatter(x=[points[i][0]], y=[points[i][1]], mode='markers', marker=dict(color=color)))
    for i in range(len(points)):
        for j in range(i+1, len(points)):
            fig.add_trace(go.Scatter(x=[points[i][0], points[j][0]], y=[points[i][1], points[j][1]], mode='lines', line=dict(color=color)))
    fig.update_yaxes(scaleanchor="x", scaleratio=1)
    return fig
...
def connect_all_with_points(points, colorp='b', colorl='g'):
    ...
    for i in range(len(points)):
        for j in range(i+1, len(points)):
            fig.add_trace(go.Scatter(x=[points[i][0], points[j][0]], y=[points[i][1], points[j][1]], mode='lines', line=dict(color=colorl)))
    for i in range(len(points)):
        fig.add_trace(go.Scatter(x=[points[i][0]], y=[points[i][1]], mode='markers', marker=dict(color=colorp)))
    fig.update_yaxes(scaleanchor="x", scaleratio=1)
    return fig
...
def connect(points, color='g'):
    ...
    for i in range(-1, num-1):
        fig.add_trace(go.Scatter(x=[points[i][0], points[i+1][0]], y=[points[i][1], points[i+1][1]], mode='lines', line=dict(color=color)))
    fig.update_yaxes(scaleanchor="x", scaleratio=1)
    return fig
...
def connect_with_points(points, colorp='b', colorl='g'):
    ...
    num = len(points)
    for i in range(-1, num-1):
        fig.add_trace(go.Scatter(x=[points[i][0], points[i+1][0]], y=[points[i][1], points[i+1][1]], mode='lines', line=dict(color=colorl)))
    for i in range(len(points)):
        fig.add_trace(go.Scatter(x=[points[i][0]], y=[points[i][1]], mode='markers', marker=dict(color=colorp)))
    fig.update_yaxes(scaleanchor="x", scaleratio=1)
    return fig
...
def draw_swastika(n, R, theta=0, color='b'):
    ...
    if n == 2:
        for i in range(4):
            fig.add_trace(go.Scatter(x=[R*np.sqrt(2)**(n-2)*np.cos(i*2*np.pi/4+theta), 0],
                                     y=[R*np.sqrt(2)**(n-2)*np.sin(i*2*np.pi/4+theta), 0], mode='lines', line=dict(color=color)))
    elif n % 2 == 1:
        for i in range(4):
            fig.add_trace(go.Scatter(x=[R*np.sqrt(2)**(n-3)*np.cos(i*2*np.pi/4+theta), 0],
                                     y=[R*np.sqrt(2)**(n-3)*np.sin(i*2*np.pi/4+theta), 0], mode='lines', line=dict(color=color)))
            fig.add_trace(go.Scatter(x=[R*np.sqrt(2)**(n-2)*np.cos(i*2*np.pi/4+np.pi/4+theta), 0],
                                     y=[R*np.sqrt(2)**(n-2)*np.sin(i*2*np.pi/4+np.pi/4+theta), 0], mode='lines', line=dict(color=color)))
    else:
        for i in range(4):
            fig.add_trace(go.Scatter(x=[R*np.sqrt(2)**(n-2)*np.cos(i*2*np.pi/4+theta), 0],
                                     y=[R*np.sqrt(2)**(n-2)*np.sin(i*2*np.pi/4+theta), 0], mode='lines', line=dict(color=color)))
            fig.add_trace(go.Scatter(x=[R*np.sqrt(2)**(n-3)*np.cos(i*2*np.pi/4+np.pi/4+theta), 0],
                                     y=[R*np.sqrt(2)**(n-3)*np.sin(i*2*np.pi/4+np.pi/4+theta), 0], mode='lines', line=dict(color=color)))
    for i in range(4):
        points = swastika(n, R, i*2*np.pi/4+theta)
        fig.add_trace(go.Scatter(x=[p[0] for p in points], y=[p[1] for p in points], mode='lines', line=dict(color=color)))
    fig.update_yaxes(scaleanchor="x", scaleratio=1)
    return fig
