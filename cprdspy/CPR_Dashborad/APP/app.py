...
# 刷新文件列表
@app.callback(
    [Output("file-list-container", "children"), Output("file-data-store", "data")],
...
def refresh_file_list(n_clicks):
    ...
    file_items = [
        html.Div(
            [
                html.Div(
                    [
                        html.Span(file["name"], style={"fontWeight": "bold"}),
                        html.Span(
                            f" ({round(file['size']/1024, 1)} KB)",
                            style={"color": "#757575", "fontSize": "0.9em"},
                        ),
    ...
    file_data = [{"path": file["path"], "name": file["name"]} for file in files]
...
# 处理文件上传
@app.callback(
    [Output("notification-area", "children"), Output("refresh-file-list", "n_clicks")],
    [Input("upload-data", "contents")],
    [State("upload-data", "filename"), State("refresh-file-list", "n_clicks")],
)
def upload_files(contents, filenames, n_clicks):
    if contents is None:
        raise PreventUpdate
    ...
    for content, filename in zip(contents, filenames):
        try:
            ...
            if not filename.endswith(".csv"):
                message.append(
                    html.Div(
                        f"错误: {filename} 不是CSV文件", style={"color": "#F44336"}
                    )
                )
                continue
            ...
            message.append(
                html.Div(f"成功上传: {filename}", style={"color": "#4CAF50"})
            )

        except Exception as e:
            logger.error(f"上传文件错误: {e}")
            message.append(
                html.Div(f"上传 {filename} 失败: {str(e)}", style={"color": "#F44336"})
            )

    # 触发文件列表刷新
    return message, (n_clicks or 0) + 1
...
@app.callback(
    [
        ...
        Output("refresh-file-list", "n_clicks", allow_duplicate=True),
    ],
    [Input({"type": "delete-file", "index": dash.dependencies.ALL}, "n_clicks")],
    [State("file-data-store", "data"), State("refresh-file-list", "n_clicks")],
    prevent_initial_call=True,
)
def delete_file(n_clicks_list, file_data, refresh_clicks):
    ...
    try:
        ...
        if index >= len(file_data):
            return (
                html.Div("错误: 文件索引无效", style={"color": "#F44336"}),
                refresh_clicks,
            )

        file_path = file_data[index]["path"]
        ...
        if os.path.exists(file_path):
            os.remove(file_path)
            return (
                html.Div(f"已删除: {file_name}", style={"color": "#4CAF50"}),
                refresh_clicks + 1,
            )
        else:
            return (
                html.Div(f"错误: 文件 {file_name} 不存在", style={"color": "#F44336"}),
                refresh_clicks,
            )
        ...
        return (
            html.Div(f"删除文件失败: {str(e)}", style={"color": "#F44336"}),
            refresh_clicks,
        )
...
@app.callback(
    ...
    [Input({"type": "load-file", "index": dash.dependencies.ALL}, "n_clicks")],
    [State("file-data-store", "data")],
    prevent_initial_call=True,
)
def load_file(n_clicks_list, file_data):
    ctx = dash.callback_context
    if not ctx.triggered:
        raise PreventUpdate
...
if __name__ == "__main__":
    port = args.port
    logger.info(f"启动Dash应用，端口: {port}")
