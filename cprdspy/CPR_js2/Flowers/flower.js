...
function flower({
    ...
    for (let i = 0; i < N; i++) {
        ...
        const petals = flowerPetal({
            R, r, n, rotation: petalRot, color, alpha, center, points, lineWidth, label, useDegree, direction,
        });

        if (petals[0]) traces.push(petals[0]);
        if (petals[1]) traces.push(petals[1]);
    }
...
function ovalPetal({
    ...
    const arc1 = ovalArc({
        a, b, angle1: betaB1, angle2: betaE1, angle: rotation,
        color, alpha, center: center1Rot, points, lineWidth, label, useDegree,
    });

    const arc2 = ovalArc({
        a, b, angle1: betaB2, angle2: betaE2, angle: rotation,
        color, alpha, center: center2Rot, points, lineWidth, label, useDegree,
    });
